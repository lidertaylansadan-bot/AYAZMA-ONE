...
export async function start(req: Request, res: Response, _next: NextFunction) {
  ...
  const { runId } = await runAgentWithPersistence(agentName, {
    userId: user.id,
    projectId,
    wizardAnswers: context?.wizardAnswers,
    extra: context?.extra
  })
  ...
  await logAuditEvent({
    ...
    metadata: { agentName, runId, context },
    req
  })
...
export async function list(req: Request, res: Response, _next: NextFunction) {
  ...
  let query = supabase
    .from('agent_runs')
    .select('id, agent_name, status, project_id, created_at, updated_at')
    .eq('user_id', user.id)
