...
import { costManager } from '../optimization/costManager.js'
import { semanticCache } from '../optimization/semanticCache.js'
...
export async function routeAiRequest(input: AiRouterInput): Promise<AiRouterOutput> {
  const effective = await resolveEffectiveAiConfig({
    userId: input.userId,
    projectId: input.projectId,
    preferences: input.preferences
  })

  const providerName = effective.provider
